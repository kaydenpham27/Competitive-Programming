...
int n, a[O], p[N], rp[N], f[O][O], ff[O][O];
...
main(){
    ...
    for (int i = 0; i < n; ++ i){
        for (int j = 0; j <= i; ++ j){
            if (ff[i][j]){
                if (j) Add(ff[i + 1][j - 1], ff[i][j]);
                if (!j) Add(ff[i + 1][j], ff[i][j]);
                Add(ff[i + 1][j + 1], 2 * ff[i][j] % mod);
            }
        }
    }
    ...
    for (int i = 0; i <= n; i += 2) b[i] = p[i] * rp[i / 2] % mod * rp[i / 2 + 1] % mod * exp(2, i / 2) % mod;

    for (int i = 1; i < n; ++ i){
        vector <int> a;
        for (int j = 0; j <= n; ++ j) a.push_back(f[i][j]);
        auto c = fft::multiply_mod(a, b, mod);

        for (int j = i + 1; j <= n; ++ j) f[i + 1][j] = c[j - 1];
    }

    int res = 0;
    for (int i = m; i <= n; ++ i){
        Add(res, b[n - i] * f[m][i] % mod);
    }
