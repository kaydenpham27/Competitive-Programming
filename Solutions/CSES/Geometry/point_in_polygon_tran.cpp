...
distF<F1, F2> distSegment(const Point<F1>& point, const Line<F2>& seg) {
  if (((point - seg.a) * seg.ab) <= 0) return 1. * dist(point, seg.a);
  if (((point - seg.b()) * seg.ab) >= 0) return 1. * dist(point, seg.b());
  return 1. * distLine(point, seg);
}
...
main(){
    ...
    for (int i = 1; i <= m; ++ i){
        ...
        Line<int> l(z, Point<int>(1000000005, 1000000004));
        ...
        for (int j = 0; j < n; ++ j){
            ...
            Line<int> Seg(p[j], p[k]);
 
            if (1. * distSegment<int, int>(z, Seg) == 0){
                onBoundary = true;
                cout << "BOUNDARY\n";
                break;
            }
            ...
            Line<int> res;
            cnt += intersectSegments<int, int, int>(l, Seg, res);
            //if (intersectSegments<int, int, int>(l, Seg, res)) cnt -= (1. * distSegment<int, int>(p[k], res) == 0);
 
            /*if (intersectSegments<int, int, int>(l, Seg, res)){
                cout << j << " " << res.a.x << " " << res.a.y << endl;
            }*/
        }
