...
struct DinicWS{
    ...
    void add_edge(int a, int b, int c, bool undirected = false) {
        edge ed;
        ...
        ed.a = b; ed.b = a; ed.f = undirected ? 0 : c; ed.c = c;
...
main() {
    ...
    for (int i = 1; i <= m; ++ i){
        ...
        flow.add_edge(u, v, w, 1);
        //flow.add_edge(v, u, w);
    }
 
    flow.dinic();
    auto res = flow.MinimumCut();
