...
void Solve(void) {
  ...
  city.find_scc();
  vector<int> topo = city.topo_order();

  int numScc = city.numSCC;
  // cout << numScc << "\n";
  vector<int> deg(numScc + 2, 0);
  for (int i = 1; i <= numScc; i ++) {
    deg[i] += city.NG[i].size();
  }
  ...
  cout << city.scc[ans].size() << "\n";
  sort(all(city.scc[ans]));
  for (auto x : city.scc[ans]) cout << x << " "; cout << "\n";
}
signed main() {
